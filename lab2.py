...
# Motion Functions
# ======================
def driveStraightCM(direction, distanceInCM):
    degrees = distanceInCM * gearRatio * degreesPerCM

    left_motor.spin_for(direction, degrees, DEGREES, 200, RPM, False)
    right_motor.spin_for(direction, degrees, DEGREES, 200, RPM, False)

    while left_motor.is_spinning() or right_motor.is_spinning():
        print("Torque: %.3f" % left_motor.torque(TorqueUnits.INLB))
        print("Torque: %.3f" % right_motor.torque(TorqueUnits.INLB))
