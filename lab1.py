...
def handleMotionComplete():
    ...
    if(current_state == DRIVING_FWD):
        print('FORWARD -> BACKWARD, light sensor at %d' % lightSensor.reflectivity(PERCENT))
        ...
        print('BACKWARD -> IDLE, light sensor at %d' % lightSensor.reflectivity(PERCENT))
        current_state = IDLE
...
def checkLight():
    ...
    retVal = False
    if lightSensor.reflectivity(PERCENT) > 6:
        retVal = True
        #print("Below threshold, %d" % lightSensor.reflectivity(PERCENT))
    #else:
...
def handleLight():
    ...
    if(current_state == DRIVING_FWD):
        ...
        drive_for(REVERSE, 5, 60)
    #else:
        #print('No change, light sensor at %d' % lightSensor.reflectivity(PERCENT))
